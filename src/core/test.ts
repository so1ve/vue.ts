...
export function createProgram(options: ts.CreateProgramOptions) {
	...
	if (options.options.extendedDiagnostics || options.options.generateTrace) {
		throw toThrow(
			"--extendedDiagnostics / --generateTrace is not supported, please run `Write Virtual Files` in VSCode to write virtual files and use `--extendedDiagnostics` / `--generateTrace` via tsc instead of vue-tsc to debug.",
		);
	}
	...
	if (state.hook) {
		...
		const languageHost: vue.TypeScriptLanguageHost = {
			workspacePath: ctx.options
				.host!.getCurrentDirectory()
				.replace(windowsPathReg, "/"),
			rootPath: ctx.options
				.host!.getCurrentDirectory()
				.replace(windowsPathReg, "/"),
		...
		const vueTsLs = ts.createLanguageService(
			languageServiceHost,
			volarTs.getDocumentRegistry(
				ts,
				ts.sys.useCaseSensitiveFileNames,
				languageHost.workspacePath,
			),
		);

		volarTs.decorateLanguageService(
			languageContext.virtualFiles,
			vueTsLs,
			false,
		);
